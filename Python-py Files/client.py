	...
	while num != 0:	
		...
		if pkt_num>wright:
			...
			if wcomplete==True:
				...
				continue
			print("Inside if for pktnum and wleft,wright = "+str(pkt_num)+","+str(wleft)+","+str(wright)+" ")
			...
			if ServerData['header']=="empty":
				...
				if opcode==3:
					ack_num = ServerData['SeqNo']
					ack_num = int(ack_num)
					print("Inside if for opcode 3 ***ACK*** with pktnum and acknum = "+str(pkt_num)+","+str(ack_num)+" ")
					acks[ack_num] = 1
...
def twhs(s,host,port,fname):
    ...
    while True:
	    ...
	    if full_text['header']=="empty":
		    ...
		    if (rep_split[0]!="ACK" or rep_split[1]!="hello"):
		        continue
		    else:
		    	break
    while True:
        ...
        while True:
        	...
        	if full_text['header']=="empty":
        		...
        		if rep_split[0]!="ACK":
        			continue
        		try:
        			if rep_split[1]!="start":
        				continue
        			else:
        				break
        		except: # we start file upload
        			continue
...
losses = []
for loss in range (1,101):
	losses.append(loss)

# call(["sudo","tc","qdisc","add","dev","lo","root","netem","loss","10%"])
...
for p_loss in losses:
	print("******************************** "+str(p_loss)+" *********************************")
	tmp = str(p_loss)+"ms"
	call(["sudo","tc","qdisc","change","dev","lo","root","netem","delay",tmp])
	s1.send()
	f=open("delay.txt", 'a')
	f.write(str(times_taken[len(times_taken)-1]))
	f.write("\n")
	f.close()
